...
def verified_text(path: Path, length: int, digest: str) -> str:
    ...
    if len(data) != length or actual != digest:
        raise SystemExit(f"{path.name} mismatch: length={len(data)} sha256={actual}")
    return data.decode("utf-8")

outer = {index: verified_text(CHUNK_DIR / f"chunk_000_part_{index}.txt", 1000, digest) for index, digest in OUTER_PART_EXPECTED.items()}
outer[1] = "".join(verified_text(CHUNK_DIR / f"chunk_000_part_1_{index:02d}.txt", 100, digest) for index, digest in enumerate(SUBPART_EXPECTED))
chunks = ["".join(outer[index] for index in range(5))] + [(CHUNK_DIR / f"chunk_{index:03d}.txt").read_text(encoding="utf-8") for index in range(1, 8)]
for index, text in enumerate(chunks):
    name = f"chunk_{index:03d}.txt"
    length, digest = CHUNK_EXPECTED[name]
    if len(text) != length or hashlib.sha256(text.encode()).hexdigest() != digest:
        raise SystemExit(f"{name} mismatch")

files: dict[str, str] = json.loads(zlib.decompress(base64.b64decode("".join(chunks), validate=True)).decode("utf-8"))
for relative, content in files.items():
    target = ROOT / relative
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text(content, encoding="utf-8")

validator = ROOT / "governance/application-planning/internal-alpha/validate_feature_packets.py"
validator_text = validator.read_text(encoding="utf-8").replace('matrix.get("stateValues", [])', 'matrix.get("stateVocabulary", [])')
validator.write_text(validator_text, encoding="utf-8")
packet = ROOT / "governance/application-planning/internal-alpha/feature-packets/MV-IA-F025_ONBOARDING_HELP_DIAGNOSTICS_AND_ISSUE_REPORTING.md"
packet_text = packet.read_text(encoding="utf-8").replace("Implementation remains dependency-gated by the active P9-06 sequence.", "Application implementation remains dependency-gated by the active P9-06 sequence.")
...
for workflow_name in ("ia-d02-005-assemble.yml", "ia-d02-005-assemble-pr.yml", "ia-d02-005-assemble-v2.yml", "ia-d02-005-assemble-v3.yml", "ia-d02-005-assemble-v4.yml", "ia-d02-005-assemble-v5.yml"):

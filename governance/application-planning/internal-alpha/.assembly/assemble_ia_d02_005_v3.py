...
PART_EXPECTED = [
    "2929fe5c222b4bfe08cf4ce03386516f45050953a9245d87ab3ccc28c2918956",
    "309cbe894983b463d2b8d20857491e37a7fa9e5b060a484fc13bfdd143c5e270",
    "7b486b525a029be1878f0897377e377d74c018ebbf5ccb9f6666cebaa2c086d3",
    "40b13369652775919f0974c31fc370eb4648377588f66d2d90b5587ab75618c7",
    "959c112c79ca76228e80e757058f1bc8e399e8ea00e544708be3d08db0ec68cb",
]

part_texts: list[str] = []
for index, expected_digest in enumerate(PART_EXPECTED):
    path = CHUNK_DIR / f"chunk_000_part_{index}.txt"
    data = path.read_bytes()
    digest = hashlib.sha256(data).hexdigest()
    print(f"part {index}: length={len(data)} sha256={digest}")
    if len(data) != 1000 or digest != expected_digest:
        raise SystemExit(
            f"chunk_000 part {index} mismatch: length={len(data)} sha256={digest}"
        )
    part_texts.append(data.decode("utf-8"))

texts: list[str] = ["".join(part_texts)]
for index in range(1, 8):
    texts.append((CHUNK_DIR / f"chunk_{index:03d}.txt").read_text(encoding="utf-8"))

for index, text in enumerate(texts):
    name = f"chunk_{index:03d}.txt"
    expected_length, expected_digest = EXPECTED[name]
    digest = hashlib.sha256(text.encode("utf-8")).hexdigest()
    print(f"{name}: length={len(text)} sha256={digest}")

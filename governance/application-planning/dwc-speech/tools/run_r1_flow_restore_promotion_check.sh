...
OUT="$BASE/runs/r1_flow_restore_promotion_$(date +%Y%m%d_%H%M%S)"
...
for f in "$CK" "$WARM" "$LEGACY" "$TOOLS/DWC_REPAIR_ACCEPTANCE_IDENTITIES_v0.1.0.py" "$TOOLS/DWC_R1_Flow_Restore_Promotion_Check_v0.1.0.py"; do
...
mkdir -p "$OUT"
python "$TOOLS/DWC_R1_Flow_Restore_Promotion_Check_v0.1.0.py" \
  --piper-src "$P" \
  --r1-checkpoint "$CK" \
...
PKG="$WIN_DL/DWC_R1_FLOW_RESTORE_PROMOTION_$(date +%Y%m%d_%H%M%S).zip"
(
  cd "$OUT"
  zip -9 "$PKG" R1_FLOW_RESTORE_PROMOTION_REPORT.json SHA256SUMS.txt console.log seed_*_gate.tsv >/dev/null
)
echo "HYBRID_CHECKPOINT=$OUT/R1_flow_restored_candidate.pt"
echo "OUTPUT_DIR=$OUT"

"""Verify DWC recovery guards and diagnostics without importing retired global termination controls."""
import hashlib
import json
import subprocess
import sys
...
CANONICAL_DOOR = 'operations/BOOTSTRAP.md'
...
for file in ('DWC_Protected_R2_v0.1.0.py', 'DWC_Protected_R2B_v0.1.0.py', 'DWC_Protected_R2C_v0.1.0.py'):
    ...
    r['passed'] = r['passed'] and 'DWC TRAINING BLOCKED' in r['stderr']
    results.append(r)
...
results.append(run([sys.executable, str(HERE / 'dwc_target_gate.py'), '--manifest', str(OUT / 'reference_manifest.json'),
                    '--pairs', str(OUT / 'contrast_pairs.json'), '--corpus-artifact', str(OUT / 'reference_manifest.json'),
                    '--output', str(OUT / 'TINY_TARGET_GATE.json')], 0))
results.append(run([sys.executable, '-m', 'compileall', '-q', str(HERE)]))
report = {
    'status': 'PASS' if all(r['passed'] for r in results) else 'FAIL',
    'checks': results,
    'training_performed': False,
    'ops3_authority_boundary': CANONICAL_DOOR,
    'global_termination_control_used': False,
}
(OUT / 'VALIDATION.json').write_text(json.dumps(report, indent=2) + '\n')
print(json.dumps({'status': report['status'], 'checks': len(results), 'ops3_authority_boundary': CANONICAL_DOOR}, indent=2))
if report['status'] != 'PASS':
    print(json.dumps(results, indent=2))
    raise SystemExit(1)

patches = {
    'bootstrap_launcher': 'run_repaired_retrain.sh exits before CUDA or output allocation while the DWC guard is active',
    'local_harness': 'dwc_retrain_repaired.py refuses non-preflight use while the DWC guard is active',
...
for p in sorted(OUT.rglob('*')):
    if p.is_file() and p.name != 'SHA256SUMS.txt':
        hashes.append(hashlib.sha256(p.read_bytes()).hexdigest() + '  ' + str(p.relative_to(OUT)))

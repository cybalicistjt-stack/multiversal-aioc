...
- 3 direct PDFs / 53 visually reviewed pages.
- 4 structured support CSVs / 4,936 rows.
- 109 explicit Investigation/Knowledge ability-tree rows.
- MV-IA-F011: 10 record families, 15 typed predicates, 24 deterministic fixtures.
- The Vanishing of Dr. Wen retained as the source-grounded reference-mystery/dossier anchor without invented missing facts.
- Direct source guidance that important information should appear in **at least two places** is preserved as an authoring warning, not a universal clue-count law.

## Final implementation-ready surface

- 16 semantic identity/state layers.
- 12 presentation profiles.
- 16 permission-safe projection groups.
- 30 governed actions: 22 authoritative mutations / 8 reads.
- 36 deterministic reference cases: all 24 F011 fixtures preserved one-to-one plus 12 deeper PPIA-09 cases.
...
- Objective truth, GM solution/conclusion, observation, claim, evidence, hypothesis and Player knowledge remain distinct.
- Visibility is not truth.
- Confidence/relevance/authenticity/source reliability are attributable annotations, not objective probabilities.
- Contradictions never auto-adjudicate truth.
- Solvability/redundancy/false-lead diagnostics remain read-only authoring findings.
- No universal clue count, deterministic mystery-quality guarantee or false-lead fairness threshold is invented.
- Reveal/withhold/revoke remains an explicit governed mutation.
- Permission filtering occurs before protected resolution, counts, search, diagnostics, realtime, export, notification or AI context.
- Evidence retains owning-domain identity; PPIA-09 does not copy Item/Creature/NPC/Vehicle/Setting/Event ownership.
- PPIA-12 retains World/Setting chronology authority.
- Research/Knowledge mechanics cannot bypass permissions or define objective truth by themselves.
- Authoritative mutations require `expected_version` + `operation_id`; ambiguous results use operation-status/current-version lookup before retry.

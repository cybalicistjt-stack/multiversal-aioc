...
Validation capacity is a **runner pool**, not a named-runner dependency. Multiple isolated self-hosted services may carry the same platform-validation labels. Repository selection/health and deterministic receipt comparison should run on disposable hosted runners when they require no workstation-local capability. Workflow concurrency is scoped to the immutable candidate head: duplicate runs of the same head cancel, while an obsolete head cannot hold a newer head behind its process lifetime. Cross-platform comparison starts only after every required platform validation succeeds. Runner queue/process state is infrastructure evidence only and never a lane-state milestone.

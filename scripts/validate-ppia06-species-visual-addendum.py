...
def main():
 ...
 need(len(o["decisions"])>=25,"owner decisions incomplete")
 ...
 for s in ["Vespin","Moravi","Rakuuta","Furashin","Kola-Ha","Toba-Madra","Arborae","Mythragara","Suula","Nekron","ManyToms","The Free","Stygian","Giantkin","Gray"]: need(s in species_names,"missing profile "+s)
 ...
 need(len(c["resolved_conflicts"])>=10 and c["unresolved_visual_conflicts"]==[],"visual conflict register incomplete")
 ...
 print(f"species_profiles={p['profile_count']} art_assets={len(assets)} resolved_conflicts={len(c['resolved_conflicts'])} qa_cases={q['case_count']}")

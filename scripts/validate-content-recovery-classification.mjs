...
const fixturePaths=[
 "governance/content-recovery/fixtures/MIXED_DOMAIN_FIXTURE.json",
 "governance/content-recovery/fixtures/RULE_EXPANSION_FIXTURE.json",
 "governance/content-recovery/fixtures/RULE_EXPANSION_2_FIXTURE.json"
];
const coverage=JSON.parse(fs.readFileSync("governance/content-recovery/pilots/PHASE2_RULE_EXPANSION_2_COVERAGE.json","utf8"));
function classify(r){
 ...
 if(["dependency-edges","dependency_edges","bindings.jsonl"].some(x=>base.includes(x))) return ["relationship-or-edge","support-record","strong","edge-member-path"];
 ...
 if(["cross_pack_reference_index","cross-pack-reference-index","source-map","source_map","origin-map","origin_map","source-to-canonical","pool-reference-map","record-ids","progression-source-record-inventory","identity-status-updates","ability-identity-candidates","cost-audit","high-impact-review","source-extracts"].some(x=>base.includes(x))) return ["reference-or-index","support-record","strong","expanded-index-member"];
 ...
 if(["validation!","summary!","open decisions!","source conflicts!","duplicates!","source dispositions!","source map!","source units!","cross-pack references!","action references!","8c references!","8c dependencies!","pool references!","bindings!","identity register!"].some(x=>locator.startsWith(x))) return ["reference-or-index","support-record","strong","support-sheet"];
 ...
 if(["canonical effects!","canonical actions!","creature traits!","creature actions!","items!","species!","vehicles!","spells!","stat blocks!","biology!","taxonomy!","movement!","senses!","defenses!","skills!","behavior profiles!","acquisition & processing!","trade goods!","shared mechanics!","rules profiles!","use & identification!"].some(x=>locator.startsWith(x))) return ["primary-game-asset-candidate","partially-mapped","strong","expanded-workbook-sheet"];
...
if(contract.format!=="multiversal-deterministic-classification-contract"||contract.version!=="1.2.0") failures.push({error:"unsupported contract"});

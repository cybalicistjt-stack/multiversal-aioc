...
PPIA_ORDER = ["PPIA-01","PPIA-02","PPIA-03","PPIA-04","PPIA-05","PPIA-12","PPIA-07","PPIA-08","PPIA-09","PPIA-10","PPIA-11","PPIA-06","PPIA-13","PPIA-14","PPIA-15","PPIA-16"]
...
def validate_ppia12(p12: dict, report: str) -> None:
...
def main() -> None:
    ...
    require(any(P12_COMPLETION_MERGE in item.get("value", "") for item in p7.get("evidence", [])) or p7["status"] == "completed_verified", "PPIA-07 must preserve or supersede PPIA-12 transition evidence")

    if p7["status"] in {"started", "in_progress"}:
        require(backlog["current_work_item_id"] == "PPIA-07", "active PPIA-07 transition requires backlog current item PPIA-07")
        ...
        require(p7["status"] == "completed_verified" and p7["active_substep"] is None, "historical PPIA-07 must be completed_verified")
        require(p7["latest_pushed_commit"] == P7_FINAL_HEAD, "historical PPIA-07 exact final head mismatch")
        require(p7["pull_request"] == 246 and p7["merge_commit"] == P7_FINAL_MERGE, "historical PPIA-07 final PR/merge mismatch")
        require(not p7["unresolved_failures"] and p7["owner_decision_required"] is False, "historical PPIA-07 has unresolved state")
        require(any("31545759090" in item.get("command", "") and item.get("status") == "passed" for item in p7["validation"]), "historical PPIA-07 completion-gate evidence missing")
        require(tranches["PPIA-07"]["status"] == "completed_verified", "historical PPIA-07 backlog state must be completed_verified")
        current_id = backlog["current_work_item_id"]
        require(current_id in PPIA_ORDER and PPIA_ORDER.index(current_id) > PPIA_ORDER.index("PPIA-07"), "historical PPIA-07 requires a downstream current PPIA item")
    ...
    print(f"ppia07_transition_mode={mode}")
    print(f"ppia07_branch={P7_BRANCH}")
    if mode == "historical_completed":
        print(f"ppia07_final_head={P7_FINAL_HEAD}")
        print(f"ppia07_final_merge={P7_FINAL_MERGE}")

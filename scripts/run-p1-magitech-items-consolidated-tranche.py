...
CONTRACT_PATH = ROOT / "governance/object-system/csv-intake/P1_MAGITECH_ITEMS_CONSOLIDATED_TRANCHE_CONTRACT.json"
...
def route(raw: dict[str, str]) -> tuple[str, str, str]:
    explicit = " ".join(raw.get(k, "") for k in ("Type", "Category", "Item_Type", "Subtype", "Class", "Form", "System_Type") if raw.get(k)).lower()
    all_text = " ".join(v for v in raw.values() if v).lower()
    text = explicit or all_text
    if any(token in text for token in ("weapon", "blade", "rifle", "pistol", "cannon", "bow")):
        domain = "magitech-weapon"
    elif any(token in text for token in ("armor", "armour", "shield", "warded suit")):
        domain = "magitech-armor"
    elif any(token in text for token in ("consumable", "potion", "elixir", "charge", "single-use", "one use")):
        domain = "magitech-consumable"
    elif any(token in text for token in ("component", "module", "core", "crystal", "rune plate", "attachment")):
        domain = "magitech-component"
    elif any(token in text for token in ("tool", "kit", "implement", "instrument")):
        domain = "magitech-tool"
    elif any(token in text for token in ("device", "machine", "apparatus", "generator", "projector")):
        domain = "magitech-device"
    else:
        domain = "magitech-item"

    subtype_routes = [
        ("arcane", ("arcane", "mana", "spell", "wizard", "sorcery")),
        ("divine", ("divine", "holy", "sacred", "cleric", "faith")),
        ("elemental", ("fire", "water", "air", "earth", "ice", "lightning", "element")),
        ("necromantic", ("necrom", "death", "undead", "soul", "bone")),
        ("technomantic", ("techno", "circuit", "computer", "digital", "cyber")),
        ("dimensional", ("dimension", "portal", "teleport", "planar", "rift")),
        ("protective", ("protect", "shield", "ward", "armor", "resistance")),
        ("utility", ("utility", "tool", "repair", "communication", "storage")),
    ]
...
for row_number, raw in rows:
    name = raw.get("Item_Name") or raw.get("Name") or raw.get("Magitech_Name") or raw.get("Device_Name") or f"source-row-{row_number}"
...
for row_number, raw in rows:
    ...
    mechanical = {
        ...
        if any(token in key.lower() for token in (
            "cost", "damage", "bonus", "penalty", "capacity", "power", "energy", "mana", "charge",
            "range", "duration", "speed", "armor", "armour", "resistance", "require", "slot", "weight",
            "cooldown", "save", "difficulty", "radius", "uses"
        ))
    }
    runtime_behaviors = ["equip-or-install", "activate-or-invoke", "consume-resource", "apply-effect", "degrade-or-discharge", "remove-or-uninstall"]
    records.append({
        ...
        "compatibilityResolution": "source constraints preserved; unspecified compatibility validated at equip, use, or installation time",
...
package = {
    "packageId": "multiversal.csv.magitech-items.532",
    "version": "0.1.0",
    "records": records
}
...
report = {
    "format": "multiversal-p1-magitech-items-consolidated-tranche-report",
    "version": "0.1.0",
...
(out / "P1_MAGITECH_ITEMS_CANONICAL_PACKAGE.json").write_text(json.dumps(package, indent=2, sort_keys=True) + "\n")
(out / "P1_MAGITECH_ITEMS_CONSOLIDATED_REPORT.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")
print(json.dumps({
    "records": len(records),
    "canonicalIdsAssigned": len(canonical_ids),

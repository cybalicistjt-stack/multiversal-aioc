...
CONTRACT_PATH = ROOT / "governance/object-system/csv-intake/P1_SYMBIOTES_CYBERNETICS_CONSOLIDATED_TRANCHE_CONTRACT.json"
...
def route(raw: dict[str, str]) -> tuple[str, str, str]:
    ...
    if any(token in text for token in ("symbiote", "symbiotic", "parasite", "bonded organism")):
        domain = "symbiote"
    elif any(token in text for token in ("prosthetic", "replacement limb", "artificial limb")):
        domain = "prosthetic"
    elif any(token in text for token in ("implant", "chip", "neural lace", "subdermal")):
        domain = "implant"
    elif any(token in text for token in ("interface", "jack", "neural link", "data port")):
        domain = "interface"
    elif any(token in text for token in ("augmentation", "enhancement", "upgrade")):
        domain = "augmentation"
    else:
        domain = "cybernetic"

    subtype_routes = [
        ("neural", ("neural", "brain", "memory", "cognitive", "psionic")),
        ("sensory", ("eye", "vision", "optic", "ear", "hearing", "sensor")),
        ("limb", ("arm", "leg", "hand", "foot", "limb")),
        ("dermal", ("skin", "dermal", "subdermal", "armor", "plating")),
        ("organ", ("heart", "lung", "kidney", "liver", "organ", "blood")),
        ("weaponized", ("weapon", "blade", "cannon", "gun", "damage")),
        ("mobility", ("movement", "speed", "jump", "flight", "swim")),
        ("medical", ("healing", "repair", "regeneration", "medical")),
        ("utility", ("tool", "utility", "storage", "communication")),
    ]
...
for row_number, raw in rows:
    name = raw.get("Item_Name") or raw.get("Name") or raw.get("Cybernetic_Name") or raw.get("Symbiote_Name") or f"source-row-{row_number}"
...
for row_number, raw in rows:
    ...
    mechanical = {
        ...
        if any(token in key.lower() for token in (
            ...
            "duration", "speed", "armor", "armour", "resistance", "require", "slot", "strain"
        ))
    }
    runtime_behaviors = ["install-or-bond", "activate", "apply-effect", "consume-resource", "degrade-or-reject", "remove-or-unbond"]
    records.append({
        ...
        "compatibilityResolution": "source constraints preserved; unspecified compatibility validated at use or installation time",
...
package = {
    "packageId": "multiversal.csv.symbiotes-cybernetics.572",
    "version": "0.1.0",
    "records": records,
}
...
report = {
    "format": "multiversal-p1-symbiotes-cybernetics-consolidated-tranche-report",
    "version": "0.1.0",
...
(out / "P1_SYMBIOTES_CYBERNETICS_CANONICAL_PACKAGE.json").write_text(json.dumps(package, indent=2, sort_keys=True) + "\n")
(out / "P1_SYMBIOTES_CYBERNETICS_CONSOLIDATED_REPORT.json").write_text(json.dumps(report, indent=2, sort_keys=True) + "\n")
print(json.dumps({
    "records": len(records),
    "canonicalIdsAssigned": len(canonical_ids),

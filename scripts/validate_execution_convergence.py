...
FAILURE_CLASSES={"feature_implementation","validation_contract","validation_infrastructure","runner_environment","repository_state","owner_only"}
BLOCKED_STATES={"blocked_control_plane","blocked_environment","blocked_owner"}
class ConvergenceError(RuntimeError):pass
def require(condition:bool,message:str)->None:
    if not condition:raise ConvergenceError(message)
def load_json(root:Path,rel:str)->dict[str,Any]:
    path=root/rel; require(path.is_file(),f"missing required file: {rel}")
...
def validate_convergence_control(control:dict[str,Any],*,status:str,context:str)->None:
    ...
    if retry is not None:
        ...
        changed=retry.get("changed_since_previous"); require(isinstance(changed,list) and any(str(item).strip() for item in changed),f"{context}: retry requires changed_since_previous evidence")
...
def check(root:Path)->dict[str,Any]:
    root=root.resolve(); require((root/POLICY_PATH).is_file(),f"missing convergence policy: {POLICY_PATH}")
    ...
    if checkpoint_rel:
        ...
        if active.get("implementation_authority") is True:validate_convergence_control(checkpoint.get("convergence_control",{}),status=str(checkpoint.get("status")),context=str(active.get("attempt_id")))
    current_authorities=authority.get("current",[]); policy_entries=[item for item in current_authorities if item.get("path")==POLICY_PATH]; require(len(policy_entries)==1 and policy_entries[0].get("lifecycle")=="CURRENT","convergence policy must be registered CURRENT")
    app_registry=workflow_registry.get("repositories",{}).get("cybalicistjt-stack/Multiversal-app",{}); live=app_registry.get("live_workflows",[]); paths={item.get("path") for item in live}
...
def main()->int:
    ...
    except (ConvergenceError,OSError) as exc:result={"schema_version":"1.0.0","validator":"scripts/validate_execution_convergence.py","status":"FAIL","error":str(exc)}

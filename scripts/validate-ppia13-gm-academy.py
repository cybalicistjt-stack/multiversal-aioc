...
def main() -> int:
    ...
    assert len(tracks) == 5
    assert [len(t["topics"]) for t in tracks] == [9, 8, 8, 10, 18]
    assert curriculum["locked_counts"] == {
        "tracks": 5,
        "total_modules": 53,
        "developed_source_modules": 35,
        "outline_only_multiversal_modules": 18,
        "initial_curated_source_backed_modules": 24,
    }
    ...
    required_multiversal = {
        "Multiversal System Mastery",
        "Advanced Multiversal Combat Design",
        "Handling Inter-Reality Travel & Causal Complexity",
        "Mastering Faction Play in Multiversal",
        "Running Multiversal Warfare and Strategic Play",
        "Creating a Living Multiversal Setting",
    }
    assert required_multiversal.issubset(set(tracks[4]["topics"]))
    ...
    grounding_text = json.dumps(curriculum["canonical_grounding"])
    for term in ("PPIA-07", "PPIA-08", "PPIA-09", "PPIA-11", "MV-IA-F025", "MV-IA-F006"):
        assert term in grounding_text

    assert cases["case_count"] == len(cases["cases"]) == 20
    names = {c["name"] for c in cases["cases"]}
    for name in ("source-precedence", "outline-gap", "worldbuilding-draft", "nonhuman-culture", "combat-design", "ai-automation", "accessibility", "f024-gap", "progress-not-gate", "multiversal-grounding"):
        assert name in names

    for term in (
        "53 top-level modules",
        "24 source-backed modules",
        "outline-only",
        "World Creation tables.PDF",
        "PPIA-14",
        "F024 Pack source gap",
        "No application runtime",
        "Teaching Library / Inspector-Action-Reference Contracts",
    ):
        assert term.lower() in inventory.lower(), f"inventory missing {term!r}"

    assert checkpoint["status"] == "started" and checkpoint["owner_decision_required"] is False
    assert checkpoint["work_item_id"] == "PPIA-13"

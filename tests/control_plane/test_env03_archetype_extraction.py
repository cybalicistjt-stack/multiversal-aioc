...
class Env03ArchetypeExtractionTests(unittest.TestCase):
    ...
    def test_all_env02_profiles_map_once_to_valid_primary_archetypes(self):
        ...
        self.assertEqual(
            {row["Environment_Definition_ID"] for row in crosswalk},
            {row["Environment_Definition_ID"] for row in env02},
        )
    ...
    def test_state_and_condition_families_are_not_archetype_identity(self):
        library = self.load_json(LIBRARY)
        ids = {item["id"] for item in library["archetypes"]}
        forbidden = {
            "ARCH-POST-APOCALYPTIC",
            "ARCH-RADIOACTIVE",
            "ARCH-OVERGROWN",
            "ARCH-FLOODED",
            "ARCH-ABANDONED",

...
class FamilyExecutionPreflightTests(_legacy.FamilyExecutionPreflightTests):
    def test_vti12_closeout_and_current_sgc_selection_are_atomic(self) -> None:
        ...
        mib16 = _legacy._load_json("governance/ai/work-state/MIB-16-attempt-001.json")

        self.assertEqual(ari["status"], "completed_verified")
        self.assertEqual(ari_backlog["status"], "completed_verified")
        self.assertEqual(ari_backlog["completed_through"], "ARI-22C")
        self.assertIsNone(ari_backlog["current_item"])
        self.assertEqual(ari_backlog["successor_selection"]["work_item_id"], "MIB-16")
        self.assertEqual(mib16["status"], "completed_verified")
        self.assertEqual(mib16["successor_selection"]["work_item_id"], "MIB-17")
        self.assertEqual(mib_backlog["completed_through"], "MIB-16")
        self.assertEqual(mib_backlog["current_item"], "MIB-17")
        self.assertEqual(mib_backlog["current_attempt"], "MIB-17-attempt-001")

        selected_item = pointer["active_attempt"]["work_item_id"]
        self.assertEqual(selected_item, "MIB-17")
        self.assertEqual(pointer["active_attempt"]["attempt_id"], "MIB-17-attempt-001")
    ...
    def test_sgc_execution_units_are_pre_sized_for_one_continue(self) -> None:
        ...
        self.assertEqual(pf["strict_successor_after_family"], "MIB-16")
        active = pointer["active_attempt"]
        self.assertEqual(active["work_item_id"], "MIB-17")
        self.assertEqual(active["status"], "selected_not_started")
        ...
        self.assertIn("ARI-22C", [row["id"] for row in units])

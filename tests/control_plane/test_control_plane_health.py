...
class FamilyExecutionPreflightTests(_legacy.FamilyExecutionPreflightTests):
    def test_ari_closeout_and_mib16_selection_are_atomic(self) -> None:
        ...
        self.assertIsNone(ari_backlog["current_attempt"])
        selected_item = pointer["active_attempt"]["work_item_id"]
        successor = ari_backlog["successor_selection"]
        self.assertEqual(selected_item, successor["work_item_id"])
        self.assertEqual(pointer["active_attempt"]["attempt_id"], successor["attempt_id"])
        ...
        self.assertEqual(mib_backlog["current_item"], selected_item)
        self.assertEqual(mib_backlog["current_attempt"], pointer["active_attempt"]["attempt_id"])
        selected_index = mib_backlog["strict_order"].index(selected_item)
        self.assertGreater(selected_index, 0)
        self.assertEqual(mib_backlog["completed_through"], mib_backlog["strict_order"][selected_index - 1])

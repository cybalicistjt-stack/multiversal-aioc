...
class ParallelLaneTests(unittest.TestCase):
 @classmethod
 def setUpClass(c):c.r=json.loads((ROOT/"governance/ai/runtime/PRODUCT_EXECUTION_LANES.json").read_text());c.by={x["lane_id"]:x for x in c.r["lane_definitions"]}
 def test_initial_cap_and_zero_active_during_maintenance(self):self.assertEqual(self.r["initial_active_cap"],4);self.assertEqual(self.r["active_product_attempts"],[]);self.assertFalse(self.r["legacy_primary_selection"]["implementation_authority"])

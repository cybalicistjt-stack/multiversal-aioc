...
class Vti03StableIdentityVersioningSynchronizationRegistrationTests(unittest.TestCase):
    def test_vti03_completed_lifecycle_and_forward_vti_chain_are_consistent(self):
        ...
        self.assertEqual(vti04["application_baseline_sha"], merge)
        current = backlog["current_item"]
        order = backlog["strict_order"]
        self.assertGreater(order.index(current), order.index("VTI-03"))
        ...
        if current != "VTI-04":
            self.assertEqual(vti04["status"], "completed_verified")
            self.assertTrue(vti04["authority_retired"])
            self.assertTrue(registry["vti_04_authority"]["retired"])
        for phrase in ("VTI-03 — Stable Identity, Versioning & Synchronization","COMPLETED_VERIFIED","VTI-04 — Rules Action & Roll Bridge","VTI-05 — Character Sheet, Item & Compendium Projection","MIB-03","Platform selection remains evidence-driven"):

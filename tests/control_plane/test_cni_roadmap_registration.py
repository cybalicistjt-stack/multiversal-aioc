...
def test_cni_is_interstitial_between_smb07_and_smb08():
    index = load_json("governance/ai/runtime/ROADMAP_INDEX.json")
    amendment = load_text("governance/application-planning/APPLICATION_IMPLEMENTATION_ROADMAP_CNI_AMENDMENT_2026-09-09.md")
    smb_amendment = load_text("governance/application-planning/system-maturation-buildout/SMB_CNI_INTERSTITIAL_INTEGRATION_AMENDMENT_2026-09-09.md")
    ...
    assert "SMB-01..07 → CNI-01..13 → SMB-08 → SMB-09" in order
    assert "SMB-07 → CNI-01..13 → SMB-08" in amendment
    assert "SMB-07 → CNI-01..13 → SMB-08" in smb_amendment

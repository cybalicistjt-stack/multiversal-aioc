...
class Alp07PlayerGmUxAccessibilityNotificationsRecognitionHistoryRegistrationTests(unittest.TestCase):
    def test_alp07_governed_start_or_closeout_is_consistent(self):
        ...
        if checkpoint["status"] == "in_progress":
            ...
            self.assertEqual(alp08["application_baseline_sha"], checkpoint["application_merge_sha"])
            if alp08["status"] != "completed_verified":
                ...
                self.assertEqual(pointer["active_attempt"]["implementation_branch"], alp08["implementation_branch"])
                self.assertEqual(pointer["active_attempt"]["implementation_authority"], alp08["implementation_authority"])
                self.assertEqual(registry["active_planning_work"]["work_item"], "ALP-08")
                self.assertEqual(registry["active_planning_work"]["state"], alp08["status"])
                self.assertEqual(index["current"]["work_item_id"], "ALP-08")
                self.assertEqual(index["current"]["status"], alp08["status"])
                self.assertEqual(runtime["active_work"]["work_item"], "ALP-08")
                self.assertEqual(runtime["active_work"]["state"], alp08["status"])
            else:

const BUILD='4.1.1-explicit-audit-state';
...
function render(){
  const s=state.status||{},published=state.publication?.publishedAt;
  const executed=Boolean(state.publication&&state.status);
  const executionBanner=executed
    ? `<section class="panel"><h2>✓ MACHINE AUDIT EXECUTED</h2><p>Generated audit results were published at ${esc(published||'an unknown time')}.</p></section>`
    : `<section class="panel" style="border:2px solid #ff5d73;background:rgba(120,0,20,.22)"><h2 style="color:#ff8b9b">AUDIT NOT YET EXECUTED</h2><p><b>The four source archives are present, but no generated audit result exists.</b> The numbers below are zero or planning data, not completed scan results.</p></section>`;
  statusView.innerHTML=`${executionBanner}<section class="hero"><span class="badge">AUTHORITATIVE FOUR-ARCHIVE CORPUS</span><h2>Part 1 + Part 2 + Part 3 + Creatures</h2><p>The archive set is complete. Machine extraction, human verification, and canonical approval remain distinct gates.</p></section>${coverage()}<section class="grid two"><div class="panel"><h3>Archive authority</h3>${archiveRows()}</div><div class="panel"><h3>Completion gates</h3>${gates()}</div></section><section class="grid two"><div class="panel"><h3>Reconciliation</h3>${reconciliation()}</div><div class="panel"><h3>Coverage receipt</h3><div class="row"><b>Page coverage</b><strong>${Number(s.pageCoveragePercent||0).toFixed(2)}%</strong></div><div class="row"><b>Published result</b><small>${esc(published||state.importedAt||'NO RESULT PUBLISHED')}</small></div><div class="row"><b>Source commit</b><small>${esc(state.publication?.sourceCommit||'Not published yet')}</small></div><p class="muted">${esc(s.authorityNote||'Automatic extraction creates review candidates only.')}</p></div></section><section class="panel"><h3>Document and table schedule</h3>${scheduleView()}</section>`;bind()
}
